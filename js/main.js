...
function render() {
  ...
  if (winner) {
    ...
    if (turn === 1) {
      messageEl.innerHTML = '<span style="color: black">BLACK\'s TURN</span>';
    }
    if (turn === -1) {
...
function handleClick(evt) {
  ...
  squareEls[parseInt(row * 8) + parseInt(col)].style.border =
    "3px dotted green";
  render();
}
...
function getAvailMoves() {
  for (let r = 0; r < board.length; r++) {
    for (let c = 0; c < board[r].length; c++) {
      ...
      if (board[r][c].color === "black" || board[r][c].king)
        board[r][c].mvDiagUpLeft = checkDiagUpLeft(r - 1, c - 1);
      if (board[r][c].color === "black" || board[r][c].king)
        board[r][c].mvDiagUpRight = checkDiagUpRight(r - 1, c + 1);
      if (board[r][c].color === "black" || board[r][c].king)
        board[r][c].jumpDiagUpLeft = checkJumpDiagUpLeft(
          r - 2,
          c - 2,
          colorToFind[board[r][c].color]
        );
      if (board[r][c].color === "black" || board[r][c].king)
        board[r][c].jumpDiagUpRight = checkJumpDiagUpRight(
          r - 2,
          c + 2,
          colorToFind[board[r][c].color]
        );
      if (board[r][c].color === "red" || board[r][c].king)
        board[r][c].mvDiagDownLeft = checkDiagDownLeft(r + 1, c - 1);
      if (board[r][c].color === "red" || board[r][c].king)
        board[r][c].mvDiagDownRight = checkDiagDownRight(r + 1, c + 1);
      if (board[r][c].color === "red" || board[r][c].king)
        board[r][c].jumpDiagDownLeft = checkJumpDiagDownLeft(
          r + 2,
          c - 2,
          colorToFind[board[r][c].color]
        );
      if (board[r][c].color === "red" || board[r][c].king)
        board[r][c].jumpDiagDownRight = checkJumpDiagDownRight(
          r + 2,
          c + 2,
          colorToFind[board[r][c].color]
        );
      if (
        board[r][c].mvDiagUpLeft ||
        board[r][c].mvDiagUpRight ||
        board[r][c].jumpDiagUpLeft ||
        board[r][c].jumpDiagUpRight ||
        board[r][c].mvDiagDownLeft ||
        board[r][c].mvDiagDownRight ||
        board[r][c].jumpDiagDownLeft ||
        board[r][c].jumpDiagDownRight
      )
        board[r][c].canMove = true;
    }
  }
}
...
function handleMovePiece(evt) {
  ...
  for (let i = 0; i < selectedPiece.moveCoordinates.length; i++) {
    ...
    if (pieceMoveRow === parseInt(row) && pieceMoveCol === parseInt(col)) {
      if (
        Math.abs(selectedPiece.coordinates[0] - pieceMoveRow) +
          Math.abs(selectedPiece.coordinates[1] - pieceMoveCol) ===
        4
      )
        jump = true;
      if (jump) {
